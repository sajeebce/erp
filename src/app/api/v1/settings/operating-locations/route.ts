import { NextRequest } from 'next/server'
import { prisma } from '@/lib/db'
import { requireAuthFromRequest, requireRoleFromRequest } from '@/lib/auth'
import {
  apiSuccess,
  apiCreated,
  apiBadRequest,
  apiConflict,
  apiNotFound,
  handleRouteError,
} from '@/lib/api-response'

export async function GET(request: NextRequest) {
  try {
    const auth = await requireAuthFromRequest(request)
    const { searchParams } = new URL(request.url)
    const search = searchParams.get('search') || ''
    const businessUnitId = searchParams.get('businessUnitId')
    const isActiveParam = searchParams.get('isActive')

    const where: Record<string, unknown> = { organizationId: auth.organizationId }
    if (search) {
      where.OR = [
        { name: { contains: search, mode: 'insensitive' } },
        { code: { contains: search, mode: 'insensitive' } },
        { address: { contains: search, mode: 'insensitive' } },
      ]
    }
    if (businessUnitId) where.businessUnitId = businessUnitId
    if (isActiveParam !== null) where.isActive = isActiveParam === 'true'

    const locations = await prisma.operatingLocation.findMany({
      where,
      include: {
        businessUnit: { select: { id: true, code: true, name: true, shortName: true } },
      },
      orderBy: { code: 'asc' },
    })

    return apiSuccess(locations)
  } catch (error) {
    return handleRouteError(error)
  }
}

export async function POST(request: NextRequest) {
  try {
    const auth = await requireRoleFromRequest(request, 'ADMIN')
    const body = await request.json()
    const { code, name, businessUnitId, address, localizedName } = body

    if (!code || !name) {
      return apiBadRequest('code and name are required')
    }

    if (businessUnitId) {
      const bu = await prisma.businessUnit.findFirst({
        where: { id: businessUnitId, organizationId: auth.organizationId },
      })
      if (!bu) return apiNotFound('Business unit not found in this organization')
    }

    const existing = await prisma.operatingLocation.findUnique({
      where: { organizationId_code: { organizationId: auth.organizationId, code: code.trim() } },
    })
    if (existing) return apiConflict(`Location code "${code}" already exists`)

    const location = await prisma.operatingLocation.create({
      data: {
        organizationId: auth.organizationId,
        businessUnitId: businessUnitId ?? null,
        code: code.trim(),
        name: name.trim(),
        address: address?.trim() ?? null,
        localizedName: localizedName ?? null,
        isActive: true,
      },
      include: {
        businessUnit: { select: { id: true, code: true, name: true, shortName: true } },
      },
    })

    return apiCreated(location)
  } catch (error) {
    return handleRouteError(error)
  }
}
