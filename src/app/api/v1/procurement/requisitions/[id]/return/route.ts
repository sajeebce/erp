...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!note) return apiBadRequest('note is required')
    ...
    if (requisition.status !== 'SUBMITTED' && requisition.status !== 'REVIEWED') {
      return apiForbidden(`Only SUBMITTED or REVIEWED requisitions can be returned. Current status: ${requisition.status}`)
    }
    ...
    const approvalResult = await processApproval(approval.instanceId, auth.userId, 'RETURN', note)
    ...
    await logAudit({
      ...
      resourceId: id,
      description: `Returned purchase requisition ${requisition.prNo} for correction: ${note}`,
